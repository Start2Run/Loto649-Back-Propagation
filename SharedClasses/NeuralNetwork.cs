    ...
    public class NeuralNetwork
    {
        ...
        public void Train(double[][] trainData, int maxEprochs, double learnRate, double momentum)
        {
            ...
            for (int i = 0; i < sequence.Length; ++i)
                sequence[i] = i;
            int errInterval = maxEprochs / 10; // interval to check error
            while (epoch < maxEprochs)
            {
                ...
                if (epoch % errInterval == 0 && epoch < maxEprochs)
                {
                    double trainErr = mse;
                    Console.WriteLine("epoch = " + epoch + "  error = " + trainErr.ToString("F4"));
                    UpdateStatus("epoch = " + epoch + "  error = " + trainErr.ToString("F4"));
                }
            }
        } // Train
        ...
        public delegate void StatusUpdateHandler(object sender, HelpFunctions.ProgressEventArgs e);
        ...
        private void UpdateStatus(string status)
        {
            ...
            HelpFunctions.ProgressEventArgs args = new HelpFunctions.ProgressEventArgs(status);
